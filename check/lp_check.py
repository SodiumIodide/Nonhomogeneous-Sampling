...
def flux_plot():
    ...
    fullconverge_data = pd.read_csv("../csv/flux_cox_uniform_full.csv")
    ...
    plt.plot(fullconverge_data['distance'], fullconverge_data['flux0'], label="Full Material 0", color='r', linestyle='-.')
    plt.plot(fullconverge_data['distance'], fullconverge_data['flux1'], label="Full Material 1", color='b', linestyle='-.')
    plt.xlabel("x")
    plt.ylabel("Flux")
    plt.xscale("linear")
    plt.yscale("log")
